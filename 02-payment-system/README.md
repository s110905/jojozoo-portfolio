...
![帳號與系統權限介面](./result-permissions.png)

> 截圖取自系統的示範資料預覽模式，不含真實員工與財務資料。

![Project Status](https://img.shields.io/badge/Status-Testing-yellow)
...
> 本專案最初為單一的「款項申請與審核系統」，在開發過程中持續擴充，現已整合為涵蓋 15 個後端模組的企業內部管理平台。系統已部署至雲端 VM 並以 PM2 常駐運行，目前處於**上線測試階段**：以內部測試環境進行流程驗證，尚未正式導入日常營運，正式網域與 HTTPS 切換亦在規劃中。
